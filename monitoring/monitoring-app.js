// =====================================================================
// GToolix Monitoring Module — Dashboard Logic & Data Application
// =====================================================================
(function () {
    'use strict';
    ...
    async function checkAuth() {
        ...
        if (!sb) {
            showLoginError('لم يتم العثور على مكتبة Supabase. يرجى التحقق من الإعدادات.');
            return;
        }
    ...
    // Verify Admin Role via profiles table RLS
    async function verifyAdmin(user) {
        ...
        try {
            ...
            if (error || !data || !data.is_admin) {
                showLoginError('حسابك لا يملك صلاحيات المسؤول (is_admin = true)');
            ...
            // Boot Dashboard Modules
            initDashboardData();
        } catch (err) {
            showLoginError('حدث خطأ أثناء التحقق من الصلاحيات.');
            showLoginOverlay();
        }
    }
    ...
    async function initDashboardData() {
        appendActivityLog('تم تسجيل الدخول بنجاح وإعادة تشغيل المستشعرات');
    ...
    // 2. Realtime Online Visitors Count
    ...
    // 3. Today's Stats (Visitors & Views)
    ...
    function setupRealtimeSubscription() {
        ...
        try {
            sb.channel('realtime-monitoring')
    ...
    if (triggerCleanupBtn) {
        triggerCleanupBtn.addEventListener('click', async () => {
            if (!confirm('هل انت متاكد من تنظيف السجلات التي يزيد عمرها عن 30 يوماً؟')) return;
            const sb = getSupabase();
            if (!sb) return;
            ...
            try {
                ...
                if (error) {
                    alert('خطأ أثناء التنظيف: ' + error.message);
                } else {
