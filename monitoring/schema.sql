-- =====================================================================
-- GToolix Monitoring Module — Complete SQL Schema & RLS Policies
-- Execute this entire script inside the Supabase SQL Editor.
-- =====================================================================

-- 1. PROFILES TABLE (Extends Supabase auth.users with Admin Flag)
...
-- Security Definer function to check if current authenticated user is admin
...
-- Trigger to create profile entry upon user signup
...
BEGIN
    ...
    VALUES (
        NEW.id,
        NEW.email,
        -- Automatically make first user admin, or default to false
...
-- Data Retention Cleanup Function (Supabase Free Tier Safety)
...
-- Enable Realtime for sessions table (Required for Realtime Online Visitors)
...
-- 2. SESSIONS POLICIES
CREATE POLICY "Anon/Public can insert sessions" ON public.sessions
    FOR INSERT WITH CHECK (true);

CREATE POLICY "Anon/Public can update session heartbeat" ON public.sessions
...
CREATE POLICY "Anon/Public can insert page_views" ON public.page_views
    FOR INSERT WITH CHECK (true);
...
CREATE POLICY "Anon/Public can insert tool_usage" ON public.tool_usage
    FOR INSERT WITH CHECK (true);

CREATE POLICY "Admins can view tool_usage" ON public.tool_usage
    FOR SELECT USING (public.is_admin());

-- 5. ERRORS POLICIES
CREATE POLICY "Anon/Public can insert errors" ON public.errors
    FOR INSERT WITH CHECK (true);
...
-- 6. FEEDBACK POLICIES
CREATE POLICY "Anon/Public can insert feedback" ON public.feedback
    FOR INSERT WITH CHECK (true);
...
CREATE POLICY "Admins can select health checks" ON public.health_checks
    FOR SELECT USING (public.is_admin());

CREATE POLICY "Anon or Admins can insert health checks" ON public.health_checks
    FOR INSERT WITH CHECK (true);
